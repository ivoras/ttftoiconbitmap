...
var fontYoffset = flag.Int("yoffset", -10, "Y offset to drawing characters")
...
var outPrefix = flag.String("outprefix", "char", "Output filename prefix")

func renderFontChar(outFileName string, tplImage image.Image, f *truetype.Font, color color.RGBA, size int, yFix bool, yOffset int, ch rune) {
...
func main() {
	...
	if *fontChars != "" {
		for _, ch := range *fontChars {
			outFileName := fmt.Sprintf("%s/%s_%x.png", *outDir, *outPrefix, int(ch))
			renderFontChar(outFileName, img, font, color, *fontSize, *fontYfix, *fontYoffset, rune(ch))
		}
	} else {
		for r := rune(0); r <= unicode.MaxRune; r++ {
			if /*fontPrivateUseArea(r) ||*/ font.Index(r) == 0 {
				continue
			}
			outFileName := fmt.Sprintf("%s/%s_%x.png", *outDir, *outPrefix, int(r))
			fmt.Println("Rendering", r, "to", outFileName)
			renderFontChar(outFileName, img, font, color, *fontSize, *fontYfix, *fontYoffset, r)
		}
	}
